...
class FullScreenDialogReporte extends StatefulWidget {
  ...
  @override
  _FullScreenDialogReporteState createState() => _FullScreenDialogReporteState(reporte);
}
                  ...
                  return DefaultTabController(
                        ...
                        appBar: AppBar(
                          ...
                          bottom: TabBar(
                            ...
                            tabs: //tabs, 
                            [
                              for(int i = 0; i < lenghtTab; i++) Tab(child: Text(reporte=="asistencia"?modelEstatus.results.elementAt(i).descripcion:reporte=="relacion"?_grupos.results.elementAt(i).nombreGrupo:"data", style: TextStyle(fontSize: 17),),),
                            ],
                          ),
                        ...
                        child: TabBarView(
                          children: <Widget>[
                            for(int j = 0; j < lenghtTab; j++) ReporteEvento(dataView: "asistencia",dataId: modelEstatus.results.elementAt(j).idEstatusInvitado),  
                          ],
                        ), 
                        /*IndexedStack(
                        index: _pageIndex,
                  ...
                  return DefaultTabController(
                        ...
                        appBar: AppBar(
                          ...
                          bottom: TabBar(
                            onTap: (int index){
                              setState(() {
                                dataJson["id"]=reporte=="asistencia"?modelEstatus.results.elementAt(index).idEstatusInvitado.toString():reporte=="relacion"?_grupos.results.elementAt(index).idGrupo.toString():"null";
                                _pageIndex = index;
                              });
                            },
                            indicatorColor: Colors.white,
                        ...
                        index: _pageIndex,
                        children: <Widget>[
                          for(int j = 0; j < lenghtTab; j++) ReporteEvento(dataView: "relacion",),
                        ],
                      ),          
                      ),
                    ),
