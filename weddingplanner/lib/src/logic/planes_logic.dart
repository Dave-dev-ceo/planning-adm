...
abstract class PlanesLogic {
  ...
  Future<int> createActividadEvento(Map<String, dynamic> actividadEvento, int idTarea);
...
class ConsultasPlanesLogic extends PlanesLogic {
  ...
  Future<ItemModelPlanes> selectPlanesPlanner() async {
    ...
    final response = await client.post(
      Uri.parse(
        confiC.url + 
        confiC.puerto +
        '/wedding/PLANES/selectPlanesPlanner'
      ),
        ...
        tarea.actividadTareaPlanner.forEach((actividad) async {
          await client.post(
            ...
            body: {
              'id_evento':idEvento.toString(),
              'id_planner':idPlanner.toString(),
              'id_evento_timing':data['data'][0]['id_evento_timing'].toString(),
              'nombre':actividad.nombreActividadPlanner.toString(),
              'descripcion':actividad.descripcionActividadPlanner.toString(),
              'visible_involucrados':actividad.visibleInvolucradosActividadPlanner.toString(),
  ...
  Future<ItemModelPlanes> selectPlanesEvento(String myQuery) async {
    ...
    final response = await client.post(
      Uri.parse(
        confiC.url + 
        confiC.puerto +
        '/wedding/PLANES/selectPlanesEvento'
      ),
      body: {'condicion':myQuery, 'id_evento':idEvento.toString(), 'id_planner':idPlanner.toString()},
      headers: {HttpHeaders.authorizationHeader:token}
    );
  ...
  @override
  Future<int> createActividadEvento(Map<String, dynamic> actividadEvento, int idTarea) async {
    // variables
    int idEvento = await _sharedPreferences.getIdEvento();
    int idPlanner = await _sharedPreferences.getIdPlanner();
    String token = await _sharedPreferences.getToken();
