...
class InvitadosBloc {
  ...
  Stream<ItemModelReporteGrupos?> get reporteGrupos =>
      _reporteGruposFetcher.stream;
  ...
  fetchAllReporteGrupos(BuildContext context) async {
    ItemModelReporteGrupos? itemModel =
        await _repository.fetchReporteGrupos(context);
...
class InvitadoBloc {
  ...
  final _invitadoFetcherAcompanante = PublishSubject<ItemModelAcompanante?>();

  Stream<ItemModelAcompanante?> get allAcompanante =>
      _invitadoFetcherAcompanante.stream;

  fetchAllInvitado(int? idInvitado, BuildContext context) async {
    ItemModelInvitado? itemModel =
        await _repository.fetchAllInvitado(idInvitado, context);
    _invitadoFetcher.sink.add(itemModel);
  }

  fetchAllAcompanante(int? idInvitado, BuildContext context) async {
    ItemModelAcompanante? itemModelAcompanante =
        await _repository.fetchAllAcompanante(idInvitado, context);
    _invitadoFetcherAcompanante.sink.add(itemModelAcompanante);
  }
